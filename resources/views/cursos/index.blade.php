...
<style>
    ...
    .table thead th { background-color: #212529 !important; color: white !important; border: none; padding: 15px; }
    .table tbody tr { transition: all 0.2s ease; }
    .table td { vertical-align: middle; }
    .expanded-section { background-color: #f8f9fa; border-left: 5px solid #0d6efd; padding: 20px; }
    .subcourse-table { background: white; border-radius: 8px; overflow: hidden; }
    ...
    <div class="table-container">
        ...
        <div class="row mb-4 align-items-center">
            ...
            <div class="col-md-8 text-end">
                <div class="d-flex justify-content-end gap-2">
                    ...
                    <a href="{{ route('exportar.cursos.excel') }}" class="btn btn-outline-success">
                        <i class="fas fa-file-excel me-1"></i> Excel
                    </a>
                    <a href="{{ route('exportar.cursos.csv') }}" class="btn btn-outline-info">
                        <i class="fas fa-file-csv me-1"></i> CSV
                    </a>
                    <a href="{{ route('curso.iniciar') }}" class="btn btn-success">
                        <i class="fas fa-plus me-1"></i> Nuevo Curso
                    </a>
                    <a href="{{ route('cursos.papelera') }}" class="btn btn-secondary">
                ...
                <tbody>
                    @forelse($cursos as $curso)
                        <tr id="row-{{ $curso->id }}">
                            ...
                            <td>{{ $curso->nombre }}</td>
                            <td>{{ \Illuminate\Support\Str::limit($curso->descripcion, 50) }}</td>
                            ...
                            <td class="text-center">
                                <div class="action-btns">
                                    <a href="{{ route('cursos.show', $curso->id) }}" class="btn btn-sm btn-info text-white" title="Ver Detalle">
                                        <i class="fas fa-eye"></i>
                                    </a>
                                    
                                    @if($curso->status == 1)
                                        <a href="{{ route('cursos.edit', $curso->id) }}" class="btn btn-sm btn-warning" title="Editar">
                                            <i class="fas fa-edit"></i>
                                        </a>
                                    @else
                                        <button class="btn btn-sm btn-warning disabled" title="Curso desactivado - No se puede editar">
                                            <i class="fas fa-edit"></i>
                                        </button>
                                    @endif
                                ...
                                <div class="expanded-section">
                                    <div class="row">
                                        <div class="col-md-5 border-end">
                                            ...
                                            <div class="mb-2"><span class="info-label">Descripción:</span> <p class="mb-1">{{ $curso->descripcion }}</p></div>
                                        ...
                                        <div class="col-md-7 ps-4">
                                            ...
                                            <div id="subcourses-list-{{ $curso->id }}" class="subcourse-table">
        ...
        <div class="d-flex justify-content-between align-items-center mt-4">
            <div class="text-muted small">
                Mostrando {{ $cursos->firstItem() ?? 0 }} a {{ $cursos->lastItem() ?? 0 }} de {{ $cursos->total() }} registros
            </div>
            <nav aria-label="Navegación de cursos">
                <ul class="pagination">
                    ...
                    @else
                        <li class="page-item"><a class="page-link" href="{{ $cursos->previousPageUrl() }}" rel="prev">Anterior</a></li>
                    @endif

                    {{-- Botón Siguiente --}}
                    @if ($cursos->hasMorePages())
                        <li class="page-item"><a class="page-link" href="{{ $cursos->nextPageUrl() }}" rel="next">Siguiente</a></li>
                    @else
...
<script>
    ...
    function loadSubcourses(parentId) {
        const container = document.getElementById(`subcourses-list-${parentId}`);
