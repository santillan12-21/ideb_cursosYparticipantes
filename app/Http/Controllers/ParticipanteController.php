    ...
    public function update(Request $request, $N)
        {
            $request->validate([
                ...
                'CursoInscrito' => 'required|array',
            ]);

            $participante = Participantes::where('N', $N)->firstOrFail();
            $participante->update($request->except('CursoInscrito'));
        ...
        public function store(Request $request)
        {
            $request->validate([
                ...
                'EstadoDePago' => 'nullable|string|max:255',
                'FechadelCurso' => 'required|date',
                'CursoInscrito' => 'required|array',
            ]);
            ...
            $participante->cursos()->attach($request->CursoInscrito, [
                'FechadelCurso' => $request->FechadelCurso,
            ]);
